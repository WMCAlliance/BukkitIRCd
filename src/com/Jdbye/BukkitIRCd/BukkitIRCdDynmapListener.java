	...
	public void onDynmapWebChatEvent(DynmapWebChatEvent webevt) {
		try {
			...
			case STANDALONE:
				...
				break;
			case INSPIRCD:
				if (IRCd.isLinkcompleted() && IRCd.msgDynmapMessage.length() > 0) {
					final String message =
							IRCd.msgDynmapMessage
							.replace("{User}", webevt.getName())
		                    .replace("{Message}", webevt.getMessage());
			...
			BukkitIRCdPlugin.log.warning("Unexpected error occurred in event " + webevt.getEventName());
			e.printStackTrace();
		}
	}
}
