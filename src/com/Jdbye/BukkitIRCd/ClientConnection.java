...
public class ClientConnection implements Runnable {
		...
		if (running) {
			try {
				...
				Thread.currentThread().setName(
						"Thread-BukkitIRCd-ClientConnection-" + ipaddress);
				...
				if ((IRCUserManagement.isBanned(nick + "!" + ident + "@"
						+ hostmask))
						|| (IRCUserManagement.isBanned(nick + "!" + ident + "@"
								+ ipaddress))) {
	...
	/**
	 * A huge beast that does a lot of work when a client connects - NEEDS
	 * CLEANUP
	 * 
	 * @param line
	 *            A message to be parsed
	 */
	private void parseMessage(String line) {
		...
		if (split[0].equalsIgnoreCase("NICK")) {
			...
			if (split.length < 2) {
				...
				if ((IRCUserManagement.getIRCUser(split[1]) != null)
				...
				if (split.length < 2) {
					...
					if (split[1].equalsIgnoreCase(Config.getIrcdChannel())) {
						...
						if (modes.contains("%") || modes.contains("@")
							...
							IRCFunctionality.setTopic(chantopic, nick,
									getFullHost());
						} else {
							// Not op
				...
				if (split.length < 2) {
					...
					if (split[1].equalsIgnoreCase(Config
									...
									if (i2 + 3 < split.length) {
										...
										if (modes.contains("%")
											...
											if (add == 1) {
												...
												IRCUserManagement.unBanIRCUser(
														host, getFullHost());
											}
						...
						if ((isOper) && (split[2].startsWith("-"))
							...
							IRCFunctionality.writeAll(":" + getFullHost()
									+ " PART " + Config.getIrcdConsoleChannel()
									+ " :De-opered");
						}
					} else {
						writeln(IRCd.serverMessagePrefix + " 401 " + nick + " "
					...
					while (i < split.length) {
						...
						if (split[i].equalsIgnoreCase(Config
							...
							hosts += targetnick + "=+" + targetident + "@"
									+ targethost + " ";
						} else if ((ircuser = IRCUserManagement
								.getIRCUser(split[i])) != null) {
				...
				if (split.length < 3) {
					...
					if (modes.contains("%") || modes.contains("@")
						...
						if ((ircuser = IRCUserManagement.getIRCUser(bannick)) != null) {
							IRCUserManagement.kickIRCUser(ircuser, nick, ident,
									hostmask, reason, false);
							...
							if ((IRCd.isPlugin())
								...
								if (reason != null) {
									...
									BukkitUserManagement.kickPlayerIngame(nick,
											bannick,
				...
				if (split.length < 3) {
					...
					if (split[1].equalsIgnoreCase(Config.getIrcdChannel())) {
						if (IRCUserManagement.isBanned(getFullHost())) {
							...
							IRCFunctionality.writeAllExcept(
									nick,
									":" + getFullHost() + " PRIVMSG "
											+ Config.getIrcdChannel() + " :"
											+ message);
						...
						IRCUser ircuser = IRCUserManagement
								.getIRCUser(split[1]);
						int user;
						if (ircuser != null) {
							IRCFunctionality.writeTo(ircuser.nick, ":"
									+ getFullHost() + " PRIVMSG " + split[1]
									+ " :" + message);
						} else if ((user = BukkitUserManagement
								.getUser(split[1])) >= 0) {
							if ((isAction || (!isCTCP)) && (IRCd.isPlugin())
									&& (BukkitIRCdPlugin.thePlugin != null)) {
								synchronized (IRCd.csBukkitPlayers) {
	...
	/**
	 * Sends non-MOTD general welcome information to the newly connected user,
	 * including compatability information
	 * 
	 * Might kick the user if they're banned
	 */
	public void sendWelcome() {
		if ((IRCUserManagement.isBanned(nick + "!" + ident + "@" + hostmask))
				|| (IRCUserManagement.isBanned(nick + "!" + ident + "@"
						+ ipaddress))) {
			writeln("ERROR :Closing Link: [" + ipaddress
					+ "] (You are banned from this server)");
			...
			Thread.currentThread().setName(
					"Thread-BukkitIRCd-ClientConnection-" + nick);
	...
	/**
	 * Connects the user to the channel and sends the topic, modes, and names of
	 * other users
	 * 
	 * @param channelName
	...
	public void sendChanWelcome(String channelName) {
		if (channelName.equalsIgnoreCase(Config.getIrcdConsoleChannel())) {
			IRCFunctionality.writeOpers(":" + getFullHost() + " JOIN "
					+ channelName);
		} else {
			IRCFunctionality.writeAll(":" + getFullHost() + " JOIN "
					+ channelName);
		}
	 ...
	 * 
	 * @param channelName
	 *            The channel to get the topic of
	 ...
	 * 
	 * @param channelName
	 *            The channel to get the modes of
	...
	public boolean sendChanNames(String channelName) {
		if (channelName.equalsIgnoreCase(Config.getIrcdConsoleChannel())) {
			writeln(IRCd.serverMessagePrefix + " 353 = " + nick + " "
					+ channelName + " :~" + Config.getIrcdServerName() + " "
					+ IRCUserManagement.getOpers());
		} else if (channelName.equalsIgnoreCase(Config.getIrcdChannel())) {
			writeln(IRCd.serverMessagePrefix + " 353 = " + nick + " "
					+ channelName + " :~" + Config.getIrcdServerName() + " "
					+ IRCUserManagement.getUsers());
		} else {
			return false;
		}
