...
public class ClientConnection implements Runnable {
	...
	if (running) {
		...
		if ((IRCUserManagement.isBanned(nick + "!" + ident + "@" + hostmask)) || (IRCUserManagement.isBanned(nick + "!" + ident + "@" + ipaddress))) {
			...
			try {
				...
				synchronized (IRCd.csStdOut) {
				    System.out
					    .println("[BukkitIRCd] Cleaning up connection from " + getFullHost() + " (ping timeout)");
				}
				if (isIdented && isNickSet) {
	...
	if (split[0].equalsIgnoreCase("NICK")) {
		...
		if ((IRCUserManagement.getIRCUser(split[1]) != null) || (split[1].equalsIgnoreCase(Config.getIrcdServerName()))) {
			...
			if ((IRCd.isPlugin()) &&
				...
				ChatUtils.broadcastMessage(IRCd.msgIRCNickChange
				...
				BukkitIRCdPlugin.dynmap
					.sendBroadcastToWeb(
						"IRC", IRCd.msgIRCNickChangeDynmap
						.replace("{OldNick}", nick).
							replace("{NewNick}", split[1]));
			    }
			}
		...
		synchronized (IRCd.csStdOut) {
		    System.out
			    .println("[BukkitIRCd] Cleaning up connection from " + getFullHost() + " (Quit)");
		}
	    }
	    disconnect();
	    if (quitmsg != null) {
				...
				if (split[2].substring(i, i + 1).equals("+")) {
					...
					if (modes.contains("%") ||
						...
						if (IRCd.msgIRCBan.length() > 0) {
						    ChatUtils.broadcastMessage(IRCd.msgIRCBan
							    .replace(
								    "{BannedUser}",
								    host)
							    .replace(
						...
						if (IRCd.msgIRCUnban.length() > 0) {
						    ChatUtils.broadcastMessage(IRCd.msgIRCUnban
							    .replace(
								    "{BannedUser}",
								    host)
							    .replace(
		...
		if (split.length < 3) {
			...
			if ((ircuser = IRCUserManagement.getIRCUser(bannick)) != null) {
				...
				if (reason != null) {
					...
					ChatUtils.broadcastMessage(IRCd.msgIRCKickReason
						.replace("{KickedUser}",
							bannick)
					...
					ChatUtils.broadcastMessage(IRCd.msgIRCKick
						.replace("{KickedUser}",
							bannick).replace(
							"{KickedBy}", nick));
				    }
			...
			if (IRCUserManagement.isBanned(getFullHost())) {
				...
				if (message.equalsIgnoreCase("!players") &&
					...
					if (IRCd.msgIRCAction.length() > 0) {
					    ChatUtils.broadcastMessage(IRCd.msgIRCAction
						    .replace("{User}", nick)
						    .replace(
							    "{Suffix}",
					...
					if ((BukkitIRCdPlugin.dynmap != null) &&
					    ...
					    BukkitIRCdPlugin.dynmap
						    .sendBroadcastToWeb(
							    ...
							    .replace(
								    "{Message}",
								    ChatUtils.stripIRCFormatting(message2)));
					}
				    } else {

					if (IRCd.msgIRCMessage.length() > 0) {
					    ChatUtils.broadcastMessage(IRCd.msgIRCMessage
						    .replace("{User}", nick)
						    .replace(
							    "{Suffix}",
							    IRCFunctionality.getGroupSuffix(modes))
		...
		if (split.length < 3) {
			...
			if ((IRCd.isPlugin()) &&
				...
				if (IRCd.msgIRCNotice.length() > 0) {
				    ChatUtils.broadcastMessage(IRCd.msgIRCNotice
					    .replace("{User}", nick)
					    .replace("{Prefix}",
						    IRCFunctionality.getGroupPrefix(modes))
					    .replace("{Suffix}",
			...
			if (ircuser != null) {
				...
				synchronized (IRCd.csBukkitPlayers) {
					...
					ChatUtils.sendMessage(
						...
						.replace("{Suffix}", IRCFunctionality.getGroupSuffix(modes))
						.replace("{Prefix}", IRCFunctionality.getGroupPrefix(modes))
						.replace("{Message}", ChatUtils.convertColors(message, true)));
				    }
				}
			    }
			} else {
    ...
    */
	...
	if ((IRCUserManagement.isBanned(nick + "!" + ident + "@" + hostmask)) || (IRCUserManagement.isBanned(nick + "!" + ident + "@" + ipaddress))) {
		...
		if (IRCd.msgIRCJoin.length() > 0) {
		    ChatUtils.broadcastMessage(IRCd.msgIRCJoin
			    .replace("{User}", nick)
			    .replace("{Suffix}", IRCFunctionality.getGroupSuffix(modes))
			    .replace("{Prefix}", IRCFunctionality.getGroupPrefix(modes)));
		}
    ...
    */
	...
	if (IRCd.channelTopic.length() > 0) {
		...
		try {
		    writeln(IRCd.serverMessagePrefix + " 333 " + nick + " " + channelName + " " + Config.getIrcdServerName() + " " +
			    (IRCd.dateFormat.parse(Config.getServerCreationDate()).getTime() / 1000L));
    ...
    */
	...
	synchronized (IRCd.csIrcUsers) {
		...
		if ((opersOnly && user.isOper) || (!opersOnly)) {
		    if (addAll ||
			    ChatUtils.wildCardMatch(onick, pattern) ||
			    ChatUtils.wildCardMatch(onick + "!" + oident + "@" +
				    ohost, pattern)) {
	...
	if (!opersOnly) {
		...
		for (BukkitPlayer bukkitPlayer : IRCd.bukkitPlayers) {
		    ...
		    if (addAll ||
			    ChatUtils.wildCardMatch(onick, pattern) ||
			    ChatUtils.wildCardMatch(oident, pattern) ||
			    ChatUtils.wildCardMatch(onick + "!" + oident + "@" +
				    ohost, pattern) ||
			    ChatUtils.wildCardMatch(oident + "!" + oident + "@" +
				    ohost, pattern)) {
			users.add(IRCd.serverMessagePrefix + " 352 " + nick +
				" " + channel + " " + oident + " " + ohost +
				" " + Config.getIrcdServerHostName() + " " +
