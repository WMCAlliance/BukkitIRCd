...
public final class Config {
	...
	public static void saveConfiguration() {
		try {
			...
			.info("[BukkitIRCd] Saved configuration file." +
					(isDebugModeEnabled() ? " Code BukkitIRCdPlugin742." :
							""));
		} catch (final Exception e) {
			BukkitIRCdPlugin.log
			.warning("[BukkitIRCd] Caught exception while writing settings to file: ");
			e.printStackTrace();
		}
	}
	...
	public static void setIrcdPinkTimeoutInterval(
			final int ircdPinkTimeoutInterval) {
	...
	public static void setIrcdServerDescription(
			final String ircdServerDescription) {
	...
	public static String getIrcdTopic() {
		...
		return config.getString("standalone.channel-topic", ircdTopic)
				.replace("^K", (char) 3 + "").replace("^B", (char) 2 + "")
				.replace("^I", (char) 29 + "").replace("^O", (char) 15 + "")
				.replace("^U", (char) 31 + "");
	}

	public static void setIrcdTopic(final String ircdTopic) {
		config.set(
				"standalone.channel-topic",
				ircdTopic.replace("^K", (char) 3 + "")
				.replace("^B", (char) 2 + "")
				.replace("^I", (char) 29 + "")
				.replace("^O", (char) 15 + "")
				.replace("^U", (char) 31 + ""));
	...
	public static String getIrcdTopicSetBy() {
		return config.getString("standalone.channel-topic-set-by",
				getIrcdServerName());
	}
	...
	public static String getIrcdOperPass() {
		...
		final String operpass = config.getString("standalone.oper-password",
				ircdOperPass);
		if (operpass.isEmpty()) {
	...
	public static boolean isIrcdBroadCastDeathMessages() {
		...
		return config.getBoolean("broadcast-death-messages",
