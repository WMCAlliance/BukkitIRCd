	...
	public static void enableBans() {
		if (!(new File(BukkitIRCdPlugin.thePlugin.getDataFolder(), "bans.txt"))
			...
			if (writeBans()) {
				...
				.info("[BukkitIRCd] Blank bans file created." + (Config.isDebugModeEnabled() ? " Code BukkitIRCdPlugin204." : ""));
			} else {
				BukkitIRCdPlugin.log.warning("[BukkitIRCd] Failed to create bans file." + (Config.isDebugModeEnabled() ? " Error Code BukkitIRCdPlugin205." : ""));
			}
		}
	}
	...
	public static void loadBans() {
		...
		try {
			...
			try {
				...
				while ((line = input.readLine()) != null) {
					...
					if (!line.trim().startsWith("#")) {
						try {
							...
							BukkitIRCdPlugin.log.warning("[BukkitIRCd] Invalid ban: " + line);
						}
					}
				}
			} finally {
				input.close();
				BukkitIRCdPlugin.log
				.info("[BukkitIRCd] Loaded bans file." + (Config.isDebugModeEnabled() ? " Code BukkitIRCdPlugin551." : ""));
			}
		} catch (Exception e) {
