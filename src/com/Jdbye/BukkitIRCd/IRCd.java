...
import com.Jdbye.BukkitIRCd.Utilities.UidGenerator;
...
public class IRCd implements Runnable {
							...
							if ((server != null) && server.isConnected() && (!server.isClosed()) && (!lastconnected)) {
								...
								while ((!split[0].equalsIgnoreCase("SERVER")) && (server != null) && (!server.isClosed()) && server.isConnected() && running) {
									...
									if (split[0].equalsIgnoreCase("ERROR")) {
										...
										throw new IOException("Remote host rejected connection, probably configured wrong: " + ChatUtils.join(split, " ", 1));
									} else {
										line = in.readLine();
										if (line != null) {
											split = line.split(" ");
								...
								if (split[0].equalsIgnoreCase("SERVER")) {
									...
									if ((!split[2].equals(Config.getLinkReceivePassword())) || (!split[1].equals(Config.getLinkName()))) {
										if (!split[2].equals(Config.getLinkReceivePassword())) {
											ChatUtils.println("ERROR :Invalid password.");
										} else if (!split[1].equals(Config.getLinkName())) {
											ChatUtils.println("ERROR :No configuration for hostname " + split[1]);
										}
										server.close();
								...
								if ((IRCd.isPlugin) && (BukkitIRCdPlugin.thePlugin != null)) {
									if (msgLinked.length() > 0) {
										ChatUtils.broadcastMessage(msgLinked.replace("{LinkName}", Config.getLinkName()));
									}
								}
							...
							while (running && (server != null) && server.isConnected() && (!server.isClosed())) {
								try {
									if (linkLastPingPong + (Config.getLinkTimeout() * 1000) < System.currentTimeMillis()) {
										...
										ChatUtils.println("ERROR :Ping timeout");
										server.close();
									} else {
										if (linkLastPingSent + (Config.getLinkPingInterval() * 1000) < System.currentTimeMillis()) {
											ChatUtils.println(pre + "PING " + Config.getLinkServerID() + " " + remoteSID);
											linkLastPingSent = System.currentTimeMillis();
										}
										line = in.readLine();
										...
										if ((line != null) && (line.trim().length() > 0)) {
											if (line.startsWith("ERROR ")) {
												...
												throw new IOException(
														"Remote host rejected connection, probably configured wrong: " + ChatUtils.join(split, " ", 1));
											} else {
						...
						if (lastconnected) {
							...
							if ((IRCd.isPlugin) && (BukkitIRCdPlugin.thePlugin != null) && linkcompleted) {
								if (msgDelinked.length() > 0) {
									ChatUtils.broadcastMessage(msgDelinked.replace("{LinkName}", Config.getLinkName()));
								}
							}
							lastconnected = false;
						}
	...
	public void parseLinkCommand(String command) throws IOException {
		...
		if (split[1].equalsIgnoreCase("PING")) {
			...
			linkLastPingPong = System.currentTimeMillis();
			if (split.length == 3) {
				ChatUtils.println(pre + "PONG " + split[2]);
			} else if ((split.length == 4) &&
					(split[3].equalsIgnoreCase(Integer.toString(Config.getLinkServerID())))) {
				ChatUtils.println(pre + "PONG " + Config.getLinkServerID() + " " + split[2]);
			}
			...
			throw new IOException(
					"Remote host rejected connection, probably configured wrong: " + ChatUtils.join(split, " ", 2));
			...
			String realname = ChatUtils.join(split, " ", 11);
			...
			if ((iuser = IRCUserManagement.uid2ircuser.get(UID)) != null) {
				...
				if (split.length > 2) {
					...
					iuser.awayMsg = ChatUtils.join(split, " ", 2);
				} else {
					// Remove away status
					iuser.awayMsg = "";
				}
			...
			if (split[2].equalsIgnoreCase(Config.getIrcdServerHostName())) { // Double
				...
				if ((iuser = IRCUserManagement.uid2ircuser.get(split[0])) != null) {
					ChatUtils.println(pre + "PUSH " + split[0] + " ::" + Config.getIrcdServerHostName() + " 391 " + iuser.nick + " " + Config.getIrcdServerHostName() + " :" + dateFormat.format(System.currentTimeMillis()));
				}
			}
					...
					if ((ircuser = IRCUserManagement.uid2ircuser.get(usersplit[1])) != null) {
						...
						if ((IRCd.isPlugin) && (BukkitIRCdPlugin.thePlugin != null)) {
							if (!ircuser.joined) {
								...
								{
									ChatUtils.broadcastMessage(MessageFormatter.joinIRC(msgIRCJoin, IRCFunctionality.getGroupPrefix(ircuser.getTextModes()), ircuser.nick, IRCFunctionality.getGroupSuffix(ircuser.getTextModes())));
								}
			...
			if ((ircuser = IRCUserManagement.uid2ircuser.get(split[0])) != null) {
				ircuser.realname = ChatUtils.join(split, " ", 2);
			} else {
				if (Config.isDebugModeEnabled()) {
			...
			if (split[2].equalsIgnoreCase(Config.getIrcdChannel())) {
				...
				String topic = ChatUtils.join(split, " ", 5);
			...
			if (split[2].equalsIgnoreCase(Config.getIrcdChannel())) {
				...
				String topic = ChatUtils.join(split, " ", 3);
			...
			if (success) {
				ChatUtils.println(":" + target + " IDLE " + source + " " + signedOn + " " +
						idletime);
			}
			...
			if ((ircuser = IRCUserManagement.uid2ircuser.get(split[0])) != null) {
				...
				if ((IRCd.isPlugin) && (BukkitIRCdPlugin.thePlugin != null) &&
					...
					if (msgIRCNickChange.length() > 0) {
						ChatUtils.broadcastMessage(MessageFormatter.changeNick(msgIRCNickChange, ircuser.nick, split[2], IRCFunctionality.getGroupPrefix(ircuser.getTextModes()), IRCFunctionality.getGroupSuffix(ircuser.getTextModes())));
					}
					if ((BukkitIRCdPlugin.dynmap != null) &&
							(msgIRCNickChangeDynmap.length() > 0)) {
						BukkitIRCdPlugin.dynmap.sendBroadcastToWeb("IRC", MessageFormatter.changeNickDynmap(msgIRCNickChangeDynmap, ircuser.nick, split[2]));
					}
				}
			...
			if (split[3].startsWith(Integer.toString(Config.getLinkServerID()))) {
				if (split[2].equalsIgnoreCase(Config.getIrcdChannel())) {
					if (split[3].equalsIgnoreCase(serverUID)) {
						...
						ChatUtils.println(":" + serverUID + " FMODE " +
						...
						if ((bp = BukkitUserManagement.getUserByUID(split[3])) != null) {
							...
							if (Config.isDebugModeEnabled()) {
								BukkitIRCdPlugin.log.severe("[BukkitIRCd] Bukkit Player UID " + split[3] + " not found in list. Error code IRCd2051."); // Log
							}
						}

					} else {
						if (Config.isDebugModeEnabled()) {
							BukkitIRCdPlugin.log.severe("[BukkitIRCd] UID/Config.getLinkServerID() " + split[0] + " not found in list. Error code IRCd2053."); // Log
						}
					}

				} else if (split[2].equalsIgnoreCase(Config.getIrcdConsoleChannel())) {
					if (split[3].equalsIgnoreCase(serverUID)) {
						ChatUtils.println(pre + "FJOIN " + Config.getIrcdConsoleChannel() +
								" " + consoleChannelTS + " +nt :," +
								serverUID);
						ChatUtils.println(":" + serverUID + " FMODE " +
				...
				if (split[2].equalsIgnoreCase(Config.getIrcdChannel())) {
					...
					if (((ircuser = IRCUserManagement.uid2ircuser.get(split[0])) != null) ||
						...
						if ((ircvictim = IRCUserManagement.uid2ircuser.get(split[3])) != null) {
							...
							if ((IRCd.isPlugin) &&
								...
								if (reason != null) {
									if (msgIRCKickReason.length() > 0) {
										ChatUtils.broadcastMessage(MessageFormatter.irckickMsgReason(msgIRCKickReason, kicker, kicked, ChatUtils.convertColors(reason,true), IRCFunctionality.getGroupPrefix(ircvictim.getTextModes()), IRCFunctionality.getGroupSuffix(ircvictim.getTextModes()), IRCFunctionality.getGroupPrefix(modes), IRCFunctionality.getGroupSuffix(modes)));
									}
									if ((BukkitIRCdPlugin.dynmap != null) &&
											(msgIRCKickReasonDynmap.length() > 0)) {
										BukkitIRCdPlugin.dynmap.sendBroadcastToWeb("IRC", MessageFormatter.irckickMsgReason(msgIRCKickReasonDynmap, kicker, kicked, ChatUtils.stripIRCFormatting(reason), IRCFunctionality.getGroupPrefix(ircvictim.getTextModes()), IRCFunctionality.getGroupSuffix(ircvictim.getTextModes()), IRCFunctionality.getGroupPrefix(modes), IRCFunctionality.getGroupSuffix(modes)));
									}
								} else {
									if (msgIRCKick.length() > 0) {
										ChatUtils.broadcastMessage(MessageFormatter.irckickMsg(msgIRCKick, kicker, kicked, IRCFunctionality.getGroupPrefix(ircvictim.getTextModes()), IRCFunctionality.getGroupSuffix(ircvictim.getTextModes()), IRCFunctionality.getGroupPrefix(modes), IRCFunctionality.getGroupSuffix(modes)));
									}
									if ((BukkitIRCdPlugin.dynmap != null) &&
											(msgIRCKickDynmap.length() > 0)) {
										BukkitIRCdPlugin.dynmap.sendBroadcastToWeb("IRC", MessageFormatter.kickMsg(msgIRCKickDynmap, kicker, kicked));
									}
								}
							...
							if (Config.isDebugModeEnabled()) {
								BukkitIRCdPlugin.log.severe("[BukkitIRCd] UID " + split[3] + " not found in list. Error code IRCd2083."); // Log
							}
						}

					} else {
						if (Config.isDebugModeEnabled()) {
							BukkitIRCdPlugin.log.severe("[BukkitIRCd] UID/Config.getLinkServerID() " + split[0] + " not found in list. Error code IRCd2085."); // Log
						}
					}
					...
					if ((ircvictim = IRCUserManagement.uid2ircuser.get(split[3])) != null) {
						...
						if (Config.isDebugModeEnabled()) {
							BukkitIRCdPlugin.log.severe("[BukkitIRCd] UID " + split[3] + " not found in list. Error code IRCd2094."); // Log
						}
					}
			...
			if (split[2].equalsIgnoreCase(Config.getIrcdChannel())) {
				...
				if ((ircuser = IRCUserManagement.uid2ircuser.get(split[0])) != null) {
					if ((IRCd.isPlugin) && (BukkitIRCdPlugin.thePlugin != null)) {
						if (reason != null) {
							...
							if (msgIRCLeaveReason.length() > 0) {
								ChatUtils.broadcastMessage(MessageFormatter.ircleaveMsg(msgIRCLeaveReason, IRCFunctionality.getGroupPrefix(ircuser.getTextModes()), IRCFunctionality.getGroupSuffix(ircuser.getTextModes()), ircuser.nick, ChatUtils.convertColors(reason, true)));
							}
							if ((BukkitIRCdPlugin.dynmap != null) &&
									(msgIRCLeaveReasonDynmap.length() > 0)) {
								BukkitIRCdPlugin.dynmap.sendBroadcastToWeb("IRC", MessageFormatter.ircleaveDynmapMsg(msgIRCLeaveReasonDynmap, ircuser.nick, ChatUtils.stripIRCFormatting(reason)));
							}
						} else {

							if (msgIRCLeave.length() > 0) {
								ChatUtils.broadcastMessage(MessageFormatter.joinIRC(msgIRCLeave, IRCFunctionality.getGroupPrefix(ircuser.getTextModes()), IRCFunctionality.getGroupSuffix(ircuser.getTextModes()), ircuser.nick));
							}
							if ((BukkitIRCdPlugin.dynmap != null) &&
									(msgIRCLeaveDynmap.length() > 0)) {
								BukkitIRCdPlugin.dynmap.sendBroadcastToWeb("IRC", msgIRCLeaveDynmap.replace("{User}", ircuser.nick));
							}
						}
					...
					if (Config.isDebugModeEnabled()) {
						BukkitIRCdPlugin.log.severe("[BukkitIRCd] UID " + split[0] + " not found in list. Error code IRCd2125."); // Log
					}
				}
				...
				if ((ircuser = IRCUserManagement.uid2ircuser.get(split[0])) != null) {
					...
					if (Config.isDebugModeEnabled()) {
						BukkitIRCdPlugin.log.severe("[BukkitIRCd] UID " + split[0] + " not found in list. Error code IRCd2134."); // Log
					}
				}
			...
			if ((ircuser = IRCUserManagement.uid2ircuser.get(split[0])) != null) {
				...
				if (Config.isDebugModeEnabled()) {
					BukkitIRCdPlugin.log.severe("[BukkitIRCd] UID " + split[0] + " not found in list. Error code IRCd2166."); // Log
				}
			}
			...
			if ((((ircuser = IRCUserManagement.uid2ircuser.get(split[0])) != null)) ||
				...
				synchronized (csBukkitPlayers) {
					...
					if (split[2].equalsIgnoreCase(serverUID)) {
						ChatUtils.println(pre + "UID " + serverUID + " " +
								serverStartTime + " " +
						...
						if (world != null) {
							ChatUtils.println(pre + "METADATA " + UID +
									" swhois :is currently in " + world);
						} else {
							ChatUtils.println(pre +
									"METADATA " +
									UID +
						...
						if (ircuser2.joined) {
							...
							if (msgIRCLeaveReason.length() > 0) {
								ChatUtils.broadcastMessage(MessageFormatter.ircleaveMsg(msgIRCLeaveReason, IRCFunctionality.getGroupPrefix(ircuser.getTextModes()), IRCFunctionality.getGroupSuffix(ircuser.getTextModes()), user, ChatUtils.convertColors(reason, true)));
							}
							if ((BukkitIRCdPlugin.dynmap != null) &&
									(msgIRCLeaveReasonDynmap.length() > 0)) {
								BukkitIRCdPlugin.dynmap.sendBroadcastToWeb("IRC",MessageFormatter.ircleaveDynmapMsg(msgIRCLeaveReasonDynmap, user, ChatUtils.stripIRCFormatting(reason)));
							}
			...
			String message = ChatUtils.join(split, " ", 3);
			String msgtemplate = "";
					...
					if (split[2].equalsIgnoreCase(Config.getIrcdChannel())) { // Messaging
						...
						if ((IRCd.isPlugin) &&
							...
							if (message.equalsIgnoreCase("!players") &&
								...
								if (msgPlayerList.length() > 0) {
									...
									ChatUtils.println(":" + serverUID + " PRIVMSG " + Config.getIrcdChannel() + " :" + ChatUtils.convertColors(MessageFormatter.playerMsg(msgPlayerList, Integer.toString(count), s.substring(0, s.length() - 2)), false));
								}
							} else {
								...
								if (msgtemplate.length() > 0) {
									String msg = MessageFormatter.sendMsg(msgtemplate, IRCFunctionality.getGroupPrefix(ircuser.getTextModes()), IRCFunctionality.getGroupSuffix(ircuser.getTextModes()), ircuser.nick, ChatUtils.convertColors(message, true));
									...
									ChatUtils.broadcastMessage(msg);
								}
								if ((BukkitIRCdPlugin.dynmap != null) &&
										(msgtemplatedynmap.length() > 0)) {
									BukkitIRCdPlugin.dynmap
									.sendBroadcastToWeb("IRC", MessageFormatter.dynmapMsg(msgtemplatedynmap,  ircuser.nick, ChatUtils.stripIRCFormatting(message)));
								}
							}
						}
					} else if (split[2].equalsIgnoreCase(Config
						...
						if (message.startsWith("!") && (!isAction) &&
							...
							if (!ircuser.getConsoleTextModes().contains("o")) {
								ChatUtils.println(":" + serverUID + " NOTICE " + uidfrom + " :You are not a channel operator (or above). Command failed."); // Only
						...
						if ((IRCd.isPlugin) && (BukkitIRCdPlugin.thePlugin != null)) {
							if (isAction) {
								...
								message = ChatUtils.join(message.substring(1, message.length() - 1).split(" "), " ", 1);
							} else if (isNotice) {
							...
							if (msgtemplate.length() > 0) {
								BukkitIRCdPlugin.log.info(MessageFormatter.sendMsg(msgtemplate, IRCFunctionality.getGroupPrefix(ircuser.getTextModes()), IRCFunctionality.getGroupSuffix(ircuser.getTextModes()), ircuser.nick, ChatUtils.convertColors(message, true)));
							}
						}
						...
						if ((isAction || (!isCTCP)) && (IRCd.isPlugin) &&
							...
							synchronized (IRCd.csBukkitPlayers) {
								...
								if (msgtemplate.length() > 0) {
									ChatUtils.sendMessage(bukkitnick, MessageFormatter.sendMsg(msgtemplate, IRCFunctionality.getGroupPrefix(ircuser.getTextModes()), IRCFunctionality.getGroupSuffix(ircuser.getTextModes()), ircuser.nick, ChatUtils.convertColors(message, true)));
								}
							}
						}
					}
