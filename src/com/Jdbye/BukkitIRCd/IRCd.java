...
public class IRCd implements Runnable {
	...
	// This object registers itself as a console target and needs to be long
	// lived.
		...
		while (running) {
			...
			try {
						...
						while (running) {
							if ((IRCUserManagement.clientConnections.size() < Config
								...
								try {
									...
									if (server.isConnected()) {
										...
										connection.lastPingResponse = System
												.currentTimeMillis();
										IRCUserManagement.clientConnections
												.add(connection);
								...
								if (tickCount
									...
									IRCFunctionality.writeAll("PING :"
											+ tickCount);
								}
							}
							try {
								...
								// TODO Actually do something here - this is
								// terrible
							}
						}
					} catch (IOException e) {
				...
				if (e.toString().contains("NullPointerException")) {
					BukkitIRCdPlugin.log
							.severe("[BukkitIRCd] Shutting down due to unrecoverable error (and to avoid console spam).");
					running = false;
				}
			}
		}
	...
	public static int getClientCount() {
		if (mode == Modes.STANDALONE) {
			return IRCUserManagement.clientConnections.size()
					+ bukkitPlayers.size();
		} else {
	 ...
	 * world, String host, String ip) { if (getUser(nick) < 0) { synchronized
	 * (csBukkitPlayers) { BukkitPlayer bp = new BukkitPlayer(nick, world,
	 * modes, host, host, ip, System.currentTimeMillis() / 1000L,
	 ...
	 * Config.getIrcdIngameSuffix() + " " + bp.host + " " + bp.host + " " +
	 * bp.nick + " " + bp.ip + " " + bp.signedOn + " +or :Minecraft Player");
	 * Utils.println(":" + UID + " OPERTYPE IRC_Operator"); } else
	 * Utils.println(pre + "UID " + UID + " " + (bp.idleTime / 1000L) + " " +
	 * bp.nick + Config.getIrcdIngameSuffix() + " " + bp.host + " " + bp.host +
	 ...
	 * = modestr .substring(0, modestr.length() - 1); Utils.println(":" +
	 * serverUID + " FMODE " + Config.getIrcdChannel() + " " + channelTS + " +"
	 * + textMode + " " + modestr); } if (world != null) Utils.println(pre +
	 * "METADATA " + UID + " swhois :is currently in " + world); else
	 * Utils.println(pre + "METADATA " + UID +
	 * " swhois :is currently in an unknown world"); } } } return true; } else
	 * return false; }
	 */
	public void parseLinkCommand(String command) throws IOException {
		...
		if (split[1].equalsIgnoreCase("PING")) {
			...
			IRCUserManagement.uid2ircuser.put(UID, ircuser); // Add it to the
																// hashmap
			...
			if ((ircuser = IRCUserManagement.uid2ircuser.get(split[0])) != null) {
				...
				if (Config.isDebugModeEnabled()) {
					BukkitIRCdPlugin.log.severe("[BukkitIRCd] UID " + split[0]
							+ " not found in list. Error code IRCd1779.");
				}
			}
			...
			if ((ircusertarget = IRCUserManagement.uid2ircuser.get(split[2])) != null) {
				...
				if (Config.isDebugModeEnabled()) {
					...
					// and points to a bug in the code
					BukkitIRCdPlugin.log
			...
			if (split[2].equalsIgnoreCase(Config.getIrcdChannel())) {
				...
				for (String user : users) {
					...
					IRCUser ircuser;
					if ((ircuser = IRCUserManagement.uid2ircuser
							.get(usersplit[1])) != null) {
						ircuser.setModes(usersplit[0]);
						if ((IRCd.isPlugin)
								&& (BukkitIRCdPlugin.thePlugin != null)) {
							if (!ircuser.joined) {

								if (msgIRCJoin.length() > 0) // TODO I believe
																// fix for #45
																// would go here
								{
									Utils.broadcastMessage(msgIRCJoin
											...
											.replace(
													"{Prefix}",
													IRCFunctionality
															.getGroupPrefix(ircuser
																	.getTextModes()))
											.replace(
													"{Suffix}",
													IRCFunctionality
															.getGroupSuffix(ircuser
																	.getTextModes())));
								}
						...
						if (Config.isDebugModeEnabled()) {
							BukkitIRCdPlugin.log
									.severe("[BukkitIRCd] UID "
											+ usersplit[1]
											+ " not found in list. Error code IRCd1831.");
						}
					}
			...
			if ((ircuser = IRCUserManagement.uid2ircuser.get(split[0])) != null) {
				...
				if (Config.isDebugModeEnabled()) {
					BukkitIRCdPlugin.log.severe("[BukkitIRCd] UID " + split[0]
							+ " not found in list. Error code IRCd1861.");
				}
			}
			...
			if ((ircuser = IRCUserManagement.uid2ircuser.get(split[0])) != null) {
				...
				if (Config.isDebugModeEnabled()) {
					BukkitIRCdPlugin.log.severe("[BukkitIRCd] UID " + split[0]
							+ " not found in list. Error code IRCd1870.");
				}
			}
			...
			if (split.length >= 6) { // If it's not length 6, it's not a user
				...
				for (int i = 0; i < split[4].length(); i++) {
					...
					if (mode.equals("+")) {
						...
						if ((ircusertarget = IRCUserManagement.uid2ircuser
								.get(user)) != null) {
			...
			if (split[3].startsWith(Integer.toString(Config.getLinkServerID()))) {
				if (split[2].equalsIgnoreCase(Config.getIrcdChannel())) {
					if (split[3].equalsIgnoreCase(serverUID)) {
					...
					} else if (((ircuser = IRCUserManagement.uid2ircuser
							.get(split[0])) != null)
						...
						if ((bp = BukkitUserManagement.getUserByUID(split[3])) != null) {
							if ((IRCd.isPlugin)
									&& (BukkitIRCdPlugin.thePlugin != null)) {
								BukkitUserManagement.kickPlayerIngame(user,
										bp.nick, reason);
								BukkitUserManagement
				...
				if (split[2].equalsIgnoreCase(Config.getIrcdChannel())) {
					...
					if (((ircuser = IRCUserManagement.uid2ircuser.get(split[0])) != null)
						...
						if ((ircvictim = IRCUserManagement.uid2ircuser
								.get(split[3])) != null) {
									...
									if (msgIRCKickReason.length() > 0) {
										Utils.broadcastMessage(msgIRCKickReason
												...
												.replace(
														"{KickedPrefix}",
														IRCFunctionality
																.getGroupPrefix(ircvictim
																		.getTextModes()))
												.replace(
														"{KickedSuffix}",
														IRCFunctionality
																.getGroupSuffix(ircvictim
																		.getTextModes()))
												.replace(
														"{KickerPrefix}",
														IRCFunctionality
																.getGroupPrefix(modes))
												.replace(
														"{KickerSuffix}",
														IRCFunctionality
																.getGroupSuffix(modes)));
									}
									if ((BukkitIRCdPlugin.dynmap != null)
			...
			if ((((ircuser = IRCUserManagement.uid2ircuser.get(split[0])) != null))
				...
				synchronized (csBukkitPlayers) {
					...
					if (split[2].equalsIgnoreCase(serverUID)) {
						...
						Utils.println(":" + serverUID
								+ " OPERTYPE Network_Service");
						Utils.println(pre + "FJOIN "
								+ Config.getIrcdConsoleChannel() + " "
								+ consoleChannelTS + " +nt :," + serverUID);
						Utils.println(":" + serverUID + " FMODE "
						...
						Utils.println(":" + serverUID + " FMODE "
								...
								+ " " + serverUID + " " + serverUID + " "
								+ serverUID);
					} else if ((bp = BukkitUserManagement
							.getUserByUID(split[2])) != null) {
					...
					} else if ((ircuser2 = IRCUserManagement.uid2ircuser
							.get(split[2])) != null) {
						String reason;
						reason = Utils.join(split, " ", 3);
						if (reason.startsWith(":")) {
				...
				synchronized (csBukkitPlayers) {
					...
					if (split[2].equalsIgnoreCase(Config.getIrcdChannel())) { // Messaging
						...
						if ((IRCd.isPlugin)
							...
							.getUserByUID(split[2])) != null) { // Messaging
						// an
						// ingame
						// user
