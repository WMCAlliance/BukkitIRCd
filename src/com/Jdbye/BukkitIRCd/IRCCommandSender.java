...
public class IRCCommandSender implements CommandSender {
	...
	public void sendMessage(String message) {
		if (enabled) {
			...
			// TODO Ignore List (this is the staff channel chat, so may be the
			// wrong place for this to go)
			/*
			 * for (String nameToIngnore : IRCd.globalNameIgnoreList ) { if (
			 * !(IRCd.serverName.equals(nameToIngnore)) ) {
			 */

			ArrayList<String> lns = new ArrayList<String>();
			lns.add(message);
			if ((message.length() + Config.getIrcdConsoleChannel().length()) > 329) {
				// TODO A string[] of a random size that I've chosen is probably
				// quite inefficient.. look into a way to improve this ~WizardCM
				String[] l = new String[30];
				l = Iterables.toArray(
						Splitter.fixedLength(
								329 - Config.getIrcdConsoleChannel().length())
								.split(message), String.class);
				lns.remove(message);
				for (String i : l) {
					lns.add(i.toString());
				}
				...
				case STANDALONE:
					IRCFunctionality
							.writeOpers(":"
									...
									+ " :"
									+ Utils.convertColors(lns.get(ln)
											.toString(), false));
					break;
				case INSPIRCD:
					if (IRCd.isLinkcompleted()) {
						IRCFunctionality.privmsg(IRCd.serverUID, Config
								.getIrcdConsoleChannel(), Utils.convertColors(
								lns.get(ln).toString(), false));
					}
					break;
				}/*
				 * } }
	...
	@Override
	public boolean isOp() {
		// TODO Is this supposed to always return true? Why? Possibly related to
		// setOp which cannot change operator status of IRC users.. but why?
