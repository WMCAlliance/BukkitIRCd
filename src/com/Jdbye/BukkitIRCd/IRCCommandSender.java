		...
		if (enabled) {
	     ...
	     if ( !(IRCd.serverName.equals(nameToIngnore)) ) {*/
			...
			for(int ln = 0; ln < lns.size(); ln++) {
				switch (IRCd.mode) {
				case STANDALONE:
					IRCFunctionality.writeOpers(":" + Config.getIrcdServerName() + "!" + Config.getIrcdServerName() + "@" + Config.getIrcdServerHostName() + " PRIVMSG " + Config.getIrcdConsoleChannel() + " :" + ChatUtils.convertColors(lns.get(ln).toString(), false));
					break;
				case INSPIRCD:
					if (IRCd.isLinkcompleted()) {
						IRCFunctionality.privmsg(IRCd.serverUID, Config.getIrcdConsoleChannel(), ChatUtils.convertColors(lns.get(ln).toString(), false));
					}
					break;
				}/*
	     }
