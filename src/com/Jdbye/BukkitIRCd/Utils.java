...
/**
 * A general collection of utilities used within the plugin, sometimes for
 * transferring data between IRC and game.
 */
public class Utils {
	 ...
	 * 
	 * @param input
	 *            IRC Message being send to game
	 *            <p>
	...
	/**
	 * Converts colors in a string from Minecraft to IRC, or IRC to Minecraft if
	 * specified.
	 * <p>
	 * For Minecraft -> IRC, &colour codes are converted to the IRC format.
	 * <p>
	 * For IRC -> Minecraft, IRC codes are converted to Minecraft codes with the
	 * section sign.
	 * <p>
	 * 
	 * @param input
	 *            A string, usually a message, containing formatted text.
	 * @param fromIRCtoGame
	 *            If true, the string was generated in IRC and needs to be
	 *            formatted for Minecraft. If false, it was generated in
	 *            Minecraft and needs to be formatted for IRC.
	 *            <p>
	...
	public static String convertColors(String input, boolean fromIRCtoGame) {
		...
		if (fromIRCtoGame) {
			...
			output = output.replace(IRC_Color + "15", MC_Color + "7"); // IRC
																		...
																		// to MC
																		// Grey
			output = output.replace(IRC_Color + "00", MC_Color + "f"); // IRC
																		// White
																		// to MC
																		// White
	...
	public static boolean println(String... parts) {
		...
		if ((IRCd.server == null) || (!IRCd.server.isConnected())
				|| (IRCd.server.isClosed()) || (out == null)) {
			return false;
		}
	...
	// TODO Find out what this is supposed to be used for, as literally nothing
	// uses it
	/**
	 * UNKNOWN
	 * <p>
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				try {
					...
					IRCd.commandSender.sendMessage(ChatColor.ITALIC + ""
							+ ChatColor.GRAY + "[CONSOLE: Command executed.]");
				} catch (CommandException c) {
					Throwable e = c.getCause();

					IRCd.commandSender.sendMessage(ChatColor.RED
							+ "Exception in command \"" + command + "\": " + e);
					if (Config.isDebugModeEnabled()) {
