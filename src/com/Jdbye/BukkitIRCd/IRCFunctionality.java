...
public class IRCFunctionality {
	...
	/**
	 * Configure the channel topic, both main and staff if it's standalone, only
	 * main if it's linked
	 * 
	 * @param topic
	 *            The topic for the channel
	 * @param user
	 *            The username of the person setting the topic
	 * @param userhost
	...
	public static void setTopic(String topic, String user, String userhost) {
		...
		if (mode == Modes.STANDALONE) {
			...
			if ((bp = BukkitUserManagement.getUserObject(user)) != null) {
				Utils.println(":" + bp.getUID() + " TOPIC "
						+ Config.getIrcdChannel() + " :" + channelTopic);
			}
		}
	}
	...
	public static void disconnectServer(String reason) {
		...
		if (IRCd.listener != null) {
			try {
				...
				// TODO Original developer left this blank, we want to catch
				// errors properly, or not cause them at all
			}
		}
	}
	...
	public static boolean sendLinkCAPAB() {
		...
		// Utils.println("CAPAB USERMODES :bot=B oper=o u_registered=r"); //
		// Don't
	...
	/**
	 * Sends the required link burst information, including the version of the
	 * plugin, UID, and the opertype (network service)
	 * 
	...
	public static boolean writeTo(String nick, String line) {
		synchronized (csIrcUsers) {
			if (mode == Modes.STANDALONE) {
				Iterator<ClientConnection> iter = IRCUserManagement.clientConnections
						.iterator();
				while (iter.hasNext()) {
