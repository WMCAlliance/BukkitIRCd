...
public class IRCFunctionality {
		...
		synchronized (csServer) {
			if (mode == Modes.INSPIRCD) {
				if ((server != null) && server.isConnected()) {
					ChatUtils.println(pre + "SQUIT " + Config.getLinkServerID() + " :" + reason);
					if (linkcompleted) {
						if (reason != null && msgDelinkedReason.length() > 0) {
							ChatUtils.broadcastMessage(MessageFormatter.delinkedMsg(msgDelinkedReason, Config.getLinkName(), reason));
						}
						linkcompleted = false;
					}
					try {
	...
	public static boolean sendLinkCAPAB() {
		...
		ChatUtils.println("CAPAB CAPABILITIES :NICKMAX=" + (Config.getIrcdMaxNickLength() + 1) + " CHANMAX=50 IDENTMAX=33 MAXTOPIC=500 MAXQUIT=500 MAXKICK=500 MAXGECOS=500 MAXAWAY=999 MAXMODES=1 HALFOP=1 PROTOCOL=1201");
	...
	public static boolean sendLinkBurst() {
		...
		ChatUtils.println(pre + "UID " + serverUID + " " + serverStartTime + " " + Config.getIrcdServerName() + " " + Config.getIrcdServerHostName() + " " + Config.getIrcdServerHostName() + " " + Config.getIrcdServerName() + " 0.0.0.0 " + serverStartTime + " +Brok :" + BukkitIRCdPlugin.ircdVersion);
		...
		for (BukkitPlayer bp : bukkitPlayers) {
			...
			if (bp.hasPermission("bukkitircd.oper")) {
				ChatUtils.println(pre + "UID " + UID + " " + (bp.idleTime / 1000L) + " " + bp.nick + Config.getIrcdIngameSuffix() + " " + bp.realhost + " " + bp.host + " " + bp.nick + " " + bp.ip + " " + bp.signedOn + " +or :Minecraft Player");
				ChatUtils.println(":" + UID + " OPERTYPE IRC_Operator");
			} else {
				ChatUtils.println(pre + "UID " + UID + " " + (bp.idleTime / 1000L) + " " + bp.nick + Config.getIrcdIngameSuffix() + " " + bp.realhost + " " + bp.host + " " + bp.nick + " " + bp.ip + " " + bp.signedOn + " +r :Minecraft Player");
			}
	...
	public static void writeOpers(String line) {
		...
		synchronized (csIrcUsers) {
			if (mode == Modes.STANDALONE) {
				...
				while (i < IRCUserManagement.clientConnections.size()) {
					processor = IRCUserManagement.clientConnections.get(i);
					if ((processor.isConnected()) && processor.isIdented && processor.isNickSet && processor.isOper && (processor.lastPingResponse + (Config.getIrcdPinkTimeoutInterval() * 1000) > System .currentTimeMillis())) {
						processor.writeln(line);
						i++;
					} else if (!processor.running) {
						IRCUserManagement.removeIRCUser(processor.nick);
		...
		synchronized (csIrcUsers) {
			if (mode == Modes.STANDALONE) {
				...
				while (i < IRCUserManagement.clientConnections.size()) {
					...
					if ((processor.isConnected()) && processor.isIdented && processor.isNickSet && (processor.lastPingResponse + (Config.getIrcdPinkTimeoutInterval() * 1000) > System.currentTimeMillis())) {
						processor.writeln(line);
						i++;
					} else if (!processor.running) {
						IRCUserManagement.removeIRCUser(processor.nick);
