...
/**
 * Performs most of the heavy lifting in managing a BukkitPlayer. Mostly pulled
 * from IRCd.java to help clean it up. Very rough-cut, needs work.
 */
public class BukkitUserManagement {

	/**
	 * @param nick
	 *            The current player's IRC nickname
	...
	public static boolean kickBukkitUser(String kickReason, int kickedID,
		...
		if (kickedID >= 0) {
			synchronized (csBukkitPlayers) {
				...
				if (mode == Modes.STANDALONE) {
					IRCFunctionality.writeAll(":" + kickerName
							+ Config.getIrcdIngameSuffix() + "!" + kickerName
							+ "@" + kickerHost + " KICK "
							+ Config.getIrcdChannel() + " "
							+ kickedBukkitPlayer.nick
	 ...
	 * 
	 * @param kicker
	 *            The IRC user who kicked the player
	 * @param kickee
	 ...
	 * 
	 * @param modes
	 *            The IRC modes that the player should have
	 * @param player
	 *            The player in question to be added
	...
	public static boolean addBukkitUser(String modes, Player player) {
		...
		if (getUser(nick) < 0) {
			synchronized (csBukkitPlayers) {
				...
				if (mode == Modes.STANDALONE) {
					IRCFunctionality.writeAll(":" + nick
							+ Config.getIrcdIngameSuffix() + "!" + nick + "@"
	 ...
	 * @param ID
	 *            ID of the user
	 * @return Whether removing the user was successful // TODO Make the quit
	 *         message dependant on how/why they disconnect
	 */
