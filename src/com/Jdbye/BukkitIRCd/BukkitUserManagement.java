		...
		if (kickedID >= 0) {
			synchronized (csBukkitPlayers) {
				...
				if (mode == Modes.STANDALONE) {
					IRCFunctionality.writeAll(":" + Config.getIrcdServerName() + "!" + Config.getIrcdServerName() + "@" + Config.getIrcdServerHostName() + " KICK " + Config.getIrcdChannel() + " " + kickedBukkitPlayer.nick + Config.getIrcdIngameSuffix() + kickReason);
				} else {

					// KICK
					ChatUtils.println(":" + serverUID + " KICK " + Config.getIrcdChannel() + " " + kickedBukkitPlayer.nick + Config.getIrcdIngameSuffix() + kickReason);
				}
				return true;
			}
		} else {
	...
	public static boolean kickBukkitUser(String kickReason, int kickedID, int kickerID) {
		if (kickedID >= 0) {
			synchronized (csBukkitPlayers) {
				...
				if (mode == Modes.STANDALONE) {
					IRCFunctionality.writeAll(":" + kickerName + Config.getIrcdIngameSuffix() + "!" + kickerName + "@" + kickerHost + " KICK " + Config.getIrcdChannel() + " " + kickedBukkitPlayer.nick + Config.getIrcdIngameSuffix() + ChatUtils.convertColors(kickReason, false));
				} else {

					// KICK
					ChatUtils.println(":" + kickerBukkitPlayer.getUID() + " KICK " + Config.getIrcdChannel() + " " + kickedBukkitPlayer.nick + Config.getIrcdIngameSuffix() + ChatUtils.convertColors(kickReason, false));
				}
				return true;
			}
		} else {
			...
			new BukkitRunnable() {
				...
				public void run() {
					...
					if (player != null) {
						...
						if (kickReason == null || kickReason == kicker) {
							kickText = MessageFormatter.kickMsgDisplay(msgIRCKickDisplay, kicker, kickReason);

						} else {
							kickText = MessageFormatter.kickMsgDisplay(msgIRCKickDisplayReason, kicker, kickReason);
						}
			...
			synchronized (csBukkitPlayers) {
				...
				if (mode == Modes.INSPIRCD) {
					...
					synchronized (csBukkitPlayers) {
						...
						ChatUtils.println(pre + "METADATA", UID, "ssl_cert", ":vtrsE The peer did not send any certificate.");

						// Join in-game channel with modes set
						ChatUtils.println(pre + "FJOIN", Config.getIrcdChannel(), Long.toString(channelTS), "+nt", ":" + bp.getTextMode() + "," + UID);
	...
	public static boolean removeBukkitUser(int ID) {
		synchronized (csBukkitPlayers) {
			if (ID >= 0) {
				BukkitPlayer bp = bukkitPlayers.get(ID);
				if (mode == Modes.STANDALONE) {
					IRCFunctionality.writeAll(":" + bp.nick + Config.getIrcdIngameSuffix() + "!" + bp.nick + "@" + bp.host + " QUIT :" + IRCd.userDisconnectMsg);
